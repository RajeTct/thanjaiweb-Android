...
@Component({
  selector: 'app-all-blood-required',
  templateUrl: './all-blood-required.component.html',
  styleUrls: ['./all-blood-required.component.scss'],
  animations: [
    trigger('staggerIn', [
      transition('* => *', [
...
export class AllBloodRequiredComponent extends BasePage {
  ...
  ngOnInit() {
    ...
    let bloodTypes = "1,2,3,4,5,6,7,8";
    this.getAllBloodRequired(bloodTypes);
  }
  ...
  //Get Blood require details 
  getAllBloodRequired(event: any) {
    this.clientService.getAllBloodRequired(event, 28).subscribe(
      async (data: any) => {
        this.BloodRequiredArr = data.data;
      },
    );
  }
  ...
  goToDetailPage(item) {
    ...
    let URL = '/1/home/blood-required-detail' + '?' + 'Id=' + eventId;
