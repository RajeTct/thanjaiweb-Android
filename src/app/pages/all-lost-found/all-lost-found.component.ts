...
@Component({
  selector: 'app-all-lost-found',
  templateUrl: './all-lost-found.component.html',
  styleUrls: ['./all-lost-found.component.scss'],
  animations: [
    trigger('staggerIn', [
      transition('* => *', [
...
export class AllLostFoundComponent extends BasePage {
  ...
  //Used to navigate Lost-Found-Detail page
  goToDetailPage(item) {
    let eventId = item.id + '-' + '3'; //get event id
    let URL = '/1/home/lost-found-detail' + '?' + 'Id=' + eventId;
    this.router.navigateByUrl(URL);
  }

  //set events type id to and duration id to get data
  lostFoundFilter() {
    if (this.lostfound.one == true || this.lostfound.zero == true) {
      this.person = 1;
    } else {
      this.person = 0;
    }

    if (this.lostfound.two == true || this.lostfound.zero == true) {
      this.things = 2;
    } else {
      this.things = 0;
    }

    this.selectedLFTypeId = this.person + "," + this.things;
    //calling the method after getting all parameters 
    this.getLostFoundDetails(this.selectedLFTypeId);
  }


  //get lost fount details 
  getLostFoundDetails(event: any) {
    this.clientService.GetAllLostFound(event, this.cityId).subscribe(
      async (data: any) => {
