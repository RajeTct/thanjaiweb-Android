...
export class AllEventsComponent extends BasePage {
  ...
  paidClients = Array();
  paidClientDataLength: any;
  notPaidClients = Array();
  notPaidClientDataLength: any;
  cityId: any;
  categoryTypeId: any;
  ...
  searchKeyword: any;
  todayDate: any;
  topSelectedCityId: any;
  clientBasicInfo: any[];
  IsFlag: any;
  clientCategoryTypeId: any;

  eventsArr: any[];
  selectedDuration: string = '4'; // Set a default selection duraton as this month

  //for assigning values to events type for get data 
  schoolFuntion: number;
  sportsEvent: number;
  BloodCamp: number;
  MedicalCamp: number;
  CulturalCamp: number;
  ...
  EventsType = {
    zero: true,// At least one checkbox is checked by default
    one: false,
    two: false,
    three: false,
    four: false,
  ...
  constructor(injector: Injector,
    ...
    private placeService: Place, private clientService: ClientService,
  ...
  getCardColor2Search(product: any): string {
    const index = this.notPaidClients.indexOf(product);
  ...
  getFilePath(files: any[]): string {
    ...
    return fileType ? fileType.filePathwithExtension : '';
  }
  ...
  getAllClientBasicInfo() {
    ...
    this.IsFlag = IsValue.default;
    this.clientService.GetAllClientBasicInfo(this.cityId, 2, this.searchKeyword).subscribe(async (data: any) => {
  ...
  //set events type id to and duration id to get data
  Eventstypefilter() {
    ...
    if (this.EventsType.one == true || this.EventsType.zero == true) {
      this.sportsEvent = 1;
    } else {
      this.sportsEvent = 0;
    }

    if (this.EventsType.two == true || this.EventsType.zero == true) {
      this.schoolFuntion = 2;
    } else {
      this.schoolFuntion = 0;
    }

    if (this.EventsType.three == true || this.EventsType.zero == true) {
      this.BloodCamp = 3;
    } else {
      this.BloodCamp = 0;
    }

    if (this.EventsType.four == true || this.EventsType.zero == true) {
      this.MedicalCamp = 4;
    } else {
      this.MedicalCamp = 0;
    }

    if (this.EventsType.five == true || this.EventsType.zero == true) {
      this.CulturalCamp = 5;
    } else {
      this.CulturalCamp = 0;
    }

    if (this.EventsType.six == true || this.EventsType.zero == true) {
      this.SocialService = 6;
    } else {
      this.SocialService = 0;
    }

    if (this.EventsType.seven == true || this.EventsType.zero == true) {
      this.GeneralEvents = 7;
    } else {
      this.GeneralEvents = 0;
    }

    this.selectedEventTypeId = this.sportsEvent + "," + this.schoolFuntion + "," + this.BloodCamp + ","
      + this.MedicalCamp + "," + this.CulturalCamp + "," + this.SocialService + "," + this.GeneralEvents

    let DurationId = parseInt(this.selectedDuration);

    //calling the method after getting all parameters 
    this.getAllEvents(4, this.selectedEventTypeId)

  }

  //To get data for all events
  getAllEvents(Isfilter: any, selectedEventTypeId: any) {
    this.clientService.GetAllEvents(Isfilter, selectedEventTypeId, 28).subscribe(
      async (data: any) => {
        this.eventsArr = data.data;
      });
  }

  //Used to navigate Blood-Required-Detail page
  goToDetailPage(item) {
    let eventId = item.eventId + '-' + '1'; //get event id
    let URL = '/1/home/event-detail' + '?' + 'Id=' + eventId;
