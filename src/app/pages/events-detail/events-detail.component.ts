...
export class EventsDetailComponent extends BasePage {
  ...
  getEventsByEventId() {
    this.clientService.GetEventsByEventId(this.id).subscribe(async (data: any) => {
      for (var i = 0; i < data.data.length; i++) {
        this.eventsArr = data.data;
        this.eventFiles = data.data[i].eventFiles;
      }
    });
  }
